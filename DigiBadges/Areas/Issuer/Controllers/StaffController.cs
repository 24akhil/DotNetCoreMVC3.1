            ...
            try
            {
                ...
                if (useremail == null)
                {
                    ...
                    await _emailSender.SendEmailAsync(staff.Users.Email,
                                   ...
                                   $"<h3 style = 'color:blueviolet' >Congratulation, you are invited.. for login</h3><div class='text-center'><a class='btn btn-secondary' href='https://localhost:44368/Auth/Login'>Login your Account</a></div><br />" +
                                   $"" +
...
/*
        ...
        public IActionResult SelfCreated()
        {
            ...
            var userToDisplay = earnerRoleUser.Where(e => (e.CreatedBy == AppUtility.DefaultCreatedBy) || (e.CreatedBy == AppUtility.AdminRole)).ToList();

            var userid = User.Claims.FirstOrDefault(c => c.Type == AppUtility.UserId).Value;  //claim[3].Value;
            var allIssuer = _i.AsQueryable().ToList();
        ...
        public IActionResult AddAsStaff(string id)
        {
            if (id != null)
            {
                ...
                {
                    var staffilter = Builders<Issuers>.Filter.Eq("UserId", new MongoDB.Bson.ObjectId(userid));
                    var stafupdateDef = Builders<Issuers>.Update.Set("Staffsobject", staffToAdduserid);
                    stafupdateDef = stafupdateDef.Set("StaffsIds", staffToAdd.ToArray());
                    collection.UpdateOne(staffilter, stafupdateDef);
                }
                if (staffToAdduserid != null)
                {
